...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load(f"./weights/model_{EXT}.pth"))
    ...
    plt.scatter(test_img_rep2d[:, 0], test_img_rep2d[:, 1], c=Y[:], s=5)
    plt.colorbar()
    plt.show()
    plt.figure()
    ...
    with torch.no_grad():
        ...
        plt.scatter(test_rep2d[:, 0], test_rep2d[:, 1], c=Y[:], s=5)
        plt.colorbar()
        plt.show()